...
ColVis.prototype = {
	...
	"_fnDomColumnButton": function ( i )
	{
		...
		var sTitleCheck = sTitle.match(/type="checkbox"/);
		if(sTitleCheck){
			sTitle="Checkbox";
			}
		var input=$(nButton).find('input');
		if(!input.is(':checked')){
			//alert('checked');
			var classCheck="checked";
		}else{
				var classCheck="";
		}
		$(nSpan).html(
		...
		$(nButton).hover(
				function(){ 
					...
					if(input.is(':checked')){ 
						$(this).find('label').addClass('checkedHover'); 
					} else{
						$(this).find('label').addClass('hover');
						}
				},
				function(){ $(this).find('label').removeClass('hover checkedHover focus'); }
			);
	...
	"_fnDomBaseButton": function ( text )
	{
		...
		nButton.className = !this.s.dt.bJUI ? "ColVis_Button   Base TableTools_Button" :
			"ColVis_Button   Base TableTools_Button ui-button ui-state-default";
		nButton.appendChild( nSpan );
		$(nButton).attr('title','Montrer / Cacher les colonnes');
