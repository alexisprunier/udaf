...
<script type="text/javascript">
    ...
    function changeSousTheme(selectedInd){
        document.formul.soustheme.length=0;
        nouveauchoix = new Option("new");
        document.all.soustheme.options[document.all.soustheme.length] = nouveauchoix;
        document.all.new.value = "nouv";
    }    
    ...
    function save_fields(lien){
        ...
        }else{document.cookie = "commentaire_f=";}
        ...
        }else{document.cookie = "soustheme=";}
        ...
        }else{document.cookie = "txt_problematique=";}
...
<div id="creer_dossier">
    ...
    <form id="form_creer_dossier" action="accueil.php?module=dossier&amp;action=creer_dossier&amp;act=ajout" method="post" enctype="multipart/form-data">
        <div id="gauche_dossier">
            ...
            <div id="identite_client">        
                <fieldset class="ogconso" id="client">
                    ...
                    <div id="radio_button">
                        <input type="radio" id="mr" value="1" name="sexe" required  title="Sexe du client" <?php if (isset($_SESSION['sexe']) == '1') echo 'checked="checked"'; if ((isset($_GET['id'])) && ($tab_personne["sexe"] == 1)) echo 'checked="checked"';?> />
                        <label for="mr" class="label_radio">Monsieur</label> 
                        <input type="radio" id="mme" value="2" name="sexe" required  title="Sexe du client" <?php if (isset($_SESSION['sexe']) == '2') echo 'checked="checked"'; if ((isset($_GET['id'])) && ($tab_personne["sexe"] == 2)) echo 'checked="checked"';?> />
                    ...
                    <input type="text" class="inputfield" id="prenom" name="prenom" maxlength="45" required title="Pr&eacute;nom du client" value="<?php if (isset($_SESSION['prenom_p'])) echo $_SESSION['prenom_p']; if(isset($_GET['id'])) { echo $tab_personne["prenom"]; }?>"/>

                    <label for="adresse" class="lab_txt">Adresse :</label>   
                    <input type="text" class="inputfield" id="adresse" name="adresse" maxlength="80" title="Adresse du client" value="<?php if (isset($_SESSION['adr_p'])) echo $_SESSION['adr_p']; if(isset($_GET['id'])) { echo $tab_personne["adr_postale"]; }?>"/>

                    <label for="codepostal" class="lab_txt">Code Postal :</label>   
                    <input type="text" class="inputfield" id="codepostal" name="codepostal" maxlength="5" title="Code postale du client" value="<?php if (isset($_SESSION['cp_p'])) echo $_SESSION['cp_p']; if(isset($_GET['id'])) { echo $tab_personne["code_postal"]; }?>"/>

                    <label for="ville" class="lab_txt">Ville :</label>   
                    <input type="text" class="inputfield" id="ville" name="ville" maxlength="45" title="Ville du client" value="<?php if (isset($_SESSION['ville_p'])) echo $_SESSION['ville_p']; if(isset($_GET['id'])) { echo $tab_personne["ville"]; }?>"/>

                    <label for="mail" class="lab_txt">Mail :</label>   
                    <input type="text" class="inputfield" id="mail" name="mail" maxlength="45" title="Adresse mail du client" value="<?php if (isset($_SESSION['mail_p'])) echo $_SESSION['mail_p']; if(isset($_GET['id'])) { echo $tab_personne["mail"]; }?>"/>

                    <label for="telephone" class="lab_txt">T&eacute;l&eacute;phone <span class="require">*</span> :</label>   
                    <input type="text" class="inputfield" id="telephone" name="telephone" maxlength="10" required title="T&eacute;l&eacute;phone du client" value="<?php if (isset($_SESSION['fix_p'])) echo $_SESSION['fix_p']; if(isset($_GET['id'])) { echo $tab_personne["tel_fixe"]; }?>"/>

                    <label for="mobile" class="lab_txt">T&eacute;l&eacute;phone Mobile :</label>   
                    <input type="text" class="inputfield" id="mobile" name="mobile" maxlength="10" title="Autre numero de t&eacute;l&eacute;phone du client" value="<?php if (isset($_SESSION['mobil_p'])) echo $_SESSION['mobil_p']; if(isset($_GET['id'])) { echo $tab_personne["tel_port"]; }?>"/>
                </fieldset >
            </div>
            ...
            <div id="identite_fournisseur">
                <fieldset class="ogconso" id="fournisseur">
                    ...
                    <input type="text" class="inputfield" id="nom_f" name="nom_f" maxlength="45" required title="Nom de famille du fournisseur" value="<?php if (isset($_SESSION['nom_f'])) echo $_SESSION['nom_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["nom"]; }?>"/>

                    <label for="prenom_f" class="lab_txt">Pr&eacute;nom <span class="require">*</span> :</label>   
                    <input type="text" class="inputfield" id="prenom_f" name="prenom_f" maxlength="45" required title="Pr&eacute;nom du fournisseur" value="<?php if (isset($_SESSION['prenom_f'])) echo $_SESSION['prenom_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["prenom"]; }?>"/>

                    <label for="raison_sociale_f" class="lab_txt">Raison Sociale <span class="require">*</span> :</label>   
                    <input type="text" class="inputfield" id="raison_sociale_f" name="raison_sociale_f" maxlength="45" required title="Raison sociale du fournisseur" value="<?php if (isset($_SESSION['raison_sociale_f'])) echo $_SESSION['raison_sociale_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["raison_sociale"]; }?>"/>

                    <label for="adresse_f" class="lab_txt">Adresse :</label>   
                    <input type="text" class="inputfield" id="adresse_f" name="adresse_f" maxlength="80" title="Adresse du fournisseur" value="<?php if (isset($_SESSION['adresse_f'])) echo $_SESSION['adresse_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["adr_postale"]; }?>"/>

                    <label for="codepostal_f" class="lab_txt">Code Postal :</label>   
                    <input type="text" class="inputfield" id="codepostal_f" name="codepostal_f" maxlength="5" title="Code postale du fournisseur" value="<?php if (isset($_SESSION['codepostal_f'])) echo $_SESSION['codepostal_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["code_postal"]; }?>"/>

                    <label for="ville_f" class="lab_txt">Ville :</label>   
                    <input type="text" class="inputfield" id="ville_f" name="ville_f" maxlength="45" title="Ville du fournisseur" value="<?php if (isset($_SESSION['ville_f'])) echo $_SESSION['ville_f']; if(isset($_GET['id'])) {	echo $tab_fournisseur["ville"]; }?>"/>

                    <label for="mail_f" class="lab_txt">Mail :</label>   
                    <input type="text" class="inputfield" id="mail_f" name="mail_f" maxlength="45" title="Adresse e-mail du fournisseur" value="<?php if (isset($_SESSION['mail_f'])) echo $_SESSION['mail_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["mail"]; }?>"/>

                    <label for="telephone_f" class="lab_txt">T&eacute;l&eacute;phone :</label>   
                    <input type="text" class="inputfield" id="telephone_f" name="telephone_f" maxlength="10" title="T&eacute;l&eacute;phone du fournisseur" value="<?php if (isset($_SESSION['telephone_f'])) echo $_SESSION['telephone_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["tel"]; }?>"/>

                    <label for="commentaire_f" class="lab_txt">Commentaire :</label>   
                    <textarea class="comment" rows="4" cols="50" id="commentaire_f" name="commentaire_f" title="Commentaire sur le fournisseur"><?php if (isset($_SESSION['commentaire_f'])) echo $_SESSION['commentaire_f']; if(isset($_GET['id'])) { echo $tab_fournisseur["comment_fournisseur"]; }?></textarea>
                </fieldset>
            </div>
                    ...
                    <div id="form_problematique">
                        ...
                        <div id="div_theme">
                            <select class="liste_problematique" id="theme" name="theme" title="Choisir un th&egrave;me" onChange="changeSousTheme(this.selectedIndex)" required >
                                <option value="<?php if(isset($_GET['id'])) { echo $tab_theme["theme_id"]; } ?>"><?php if(isset($_GET['id'])) { echo $tab_theme["nom"]; } ?></option> 
                                <?php
                        ...
                        <div id="div_soustheme">
                            <select class="liste_problematique" id="soustheme" name="soustheme" title="Choisir un sous-th&egrave;me" required >
                                <option value="<?php if(isset($_GET['id'])) { echo $tab_sstheme["soustheme_id"]; } ?>"><?php if(isset($_GET['id'])) { echo $tab_sstheme["nom"]; } ?></option> 
                            </select>
                        </div>
                    </div>

                    <textarea id="txt_problematique" name="txt_problematique" required placeholder="D&eacute;tail de la probl&eacute;matique des deux parties"><?php if (isset($_SESSION['problematique'])) echo $_SESSION['problematique']; if(isset($_GET['id'])) { echo $dossier_en_cours["problematique"]; }?></textarea>
                </fieldset>
            </div>
        ...
        <div id="milieu">
            ...
            <div id="validation">
                <fieldset class="ogconso">
                    ...
                    <input list="users" type="text" id="list_users" placeholder="Saisir utilisateur" value="<?php echo $_SESSION["nom"]." ".$_SESSION["prenom"]; ?>">
                    <datalist id="users">
                        <option value="user1">
                        <option value="user2">
                        <option value="ahivert">
                        <option value="test">
                    </datalist>
                    <label for="check_physique" class="lab_txt">Dossier physique :</label>
                    <input id="check_physique" type="checkbox" title="Existe t-il un dossier physique?" <?php if ((isset($_GET['id'])) && ($dossier_en_cours["dossier_physique"] == 1)) echo 'checked="checked"';?>></input>
