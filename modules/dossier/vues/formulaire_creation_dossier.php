        ...
        <div id="gauche_dossier">
            ...
            <div id="identite_client">        
                <fieldset class="ogconso" id="client">
                    ...
                    <input type="text" class="inputfield" id="adresse" name="adresse" maxlength="80" title="Adresse du client" value="<?php
                    if (isset($_SESSION['adr_p']))
                        echo $_SESSION['adr_p']; if (isset($_GET['id'])) {
                        echo $tab_personne["adr_postale"];
                    }
                    ?>"/>

                    <label for="codepostal" class="lab_txt">Code Postal :</label>   
                    <input type="text" class="inputfield" id="codepostal" name="codepostal" maxlength="5" title="Code postale du client" value="<?php
                           if (isset($_SESSION['cp_p']))
                               echo $_SESSION['cp_p']; if (isset($_GET['id'])) {
                               echo $tab_personne["code_postal"];
                           }
                           ?>"/>

                    <label for="ville" class="lab_txt">Ville :</label>   
                    <input type="text" class="inputfield" id="ville" name="ville" maxlength="45" title="Ville du client" value="<?php
                           if (isset($_SESSION['ville_p']))
                               echo $_SESSION['ville_p']; if (isset($_GET['id'])) {
                               echo $tab_personne["ville"];
                           }
                           ?>"/>

                    <label for="mail" class="lab_txt">Mail :</label>   
                    <input type="text" class="inputfield" id="mail" name="mail" maxlength="45" title="Adresse mail du client" value="<?php
                           if (isset($_SESSION['mail_p']))
                               echo $_SESSION['mail_p']; if (isset($_GET['id'])) {
                               echo $tab_personne["mail"];
                           }
                           ?>"/>

                    <label for="telephone" class="lab_txt">T&eacute;l&eacute;phone <span class="require">*</span> :</label>   
                    <input type="text" class="inputfield" id="telephone" name="telephone" maxlength="10" required title="T&eacute;l&eacute;phone du client" value="<?php
                           if (isset($_SESSION['fix_p']))
                               echo $_SESSION['fix_p']; if (isset($_GET['id'])) {
                               echo $tab_personne["tel_fixe"];
                           }
                           ?>"/>

                    <label for="mobile" class="lab_txt">T&eacute;l&eacute;phone Mobile :</label>   
                    <input type="text" class="inputfield" id="mobile" name="mobile" maxlength="10" title="Autre numero de t&eacute;l&eacute;phone du client" value="<?php
                        if (isset($_SESSION['mobil_p']))
                            echo $_SESSION['mobil_p']; if (isset($_GET['id'])) {
                            echo $tab_personne["tel_port"];
                        }
                           ?>"/>
                </fieldset >
            </div>
...
?>
                    ...
                    <input type="text" class="inputfield" id="nom_f" name="nom_f" maxlength="45" required title="Nom de famille du fournisseur" value="<?php
                           if (isset($_SESSION['nom_f']))
                               echo $_SESSION['nom_f']; if (isset($_GET['id'])) {
                               echo $tab_fournisseur["nom"];
                           }
                           ?>"/>

                    <label for="prenom_f" class="lab_txt">Pr&eacute;nom <span class="require">*</span> :</label>   
                    <input type="text" class="inputfield" id="prenom_f" name="prenom_f" maxlength="45" required title="Pr&eacute;nom du fournisseur" value="<?php
                           if (isset($_SESSION['prenom_f']))
                               echo $_SESSION['prenom_f']; if (isset($_GET['id'])) {
                               echo $tab_fournisseur["prenom"];
                           }
                           ?>"/>

                    <label for="raison_sociale_f" class="lab_txt">Raison Sociale <span class="require">*</span> :</label>   
                    <input type="text" class="inputfield" id="raison_sociale_f" name="raison_sociale_f" maxlength="45" required title="Raison sociale du fournisseur" value="<?php
                           if (isset($_SESSION['raison_sociale_f']))
                               echo $_SESSION['raison_sociale_f']; if (isset($_GET['id'])) {
                               echo $tab_fournisseur["raison_sociale"];
                           }
                           ?>"/>

                    <label for="adresse_f" class="lab_txt">Adresse :</label>   
                    <input type="text" class="inputfield" id="adresse_f" name="adresse_f" maxlength="80" title="Adresse du fournisseur" value="<?php
                        if (isset($_SESSION['adresse_f']))
                            echo $_SESSION['adresse_f']; if (isset($_GET['id'])) {
                            echo $tab_fournisseur["adr_postale"];
                        }
                           ?>"/>

                    <label for="codepostal_f" class="lab_txt">Code Postal :</label>   
                    <input type="text" class="inputfield" id="codepostal_f" name="codepostal_f" maxlength="5" title="Code postale du fournisseur" value="<?php
                        if (isset($_SESSION['codepostal_f']))
                            echo $_SESSION['codepostal_f']; if (isset($_GET['id'])) {
                            echo $tab_fournisseur["code_postal"];
                        }
                           ?>"/>

                    <label for="ville_f" class="lab_txt">Ville :</label>   
                    <input type="text" class="inputfield" id="ville_f" name="ville_f" maxlength="45" title="Ville du fournisseur" value="<?php
                                if (isset($_SESSION['ville_f']))
                                    echo $_SESSION['ville_f']; if (isset($_GET['id'])) {
                                    echo $tab_fournisseur["ville"];
                                }
                                ?>"/>

                    <label for="mail_f" class="lab_txt">Mail :</label>   
                    <input type="text" class="inputfield" id="mail_f" name="mail_f" maxlength="45" title="Adresse e-mail du fournisseur" value="<?php
                                if (isset($_SESSION['mail_f']))
                                    echo $_SESSION['mail_f']; if (isset($_GET['id'])) {
                                    echo $tab_fournisseur["mail"];
                                }
                                ?>"/>

                    <label for="telephone_f" class="lab_txt">T&eacute;l&eacute;phone :</label>   
                    <input type="text" class="inputfield" id="telephone_f" name="telephone_f" maxlength="10" title="T&eacute;l&eacute;phone du fournisseur" value="<?php
                                            if (isset($_SESSION['telephone_f']))
                                                echo $_SESSION['telephone_f']; if (isset($_GET['id'])) {
                                                echo $tab_fournisseur["tel"];
                                            }
                                ?>"/>

                    <label for="commentaire_f" class="lab_txt">Commentaire :</label>   
                    <textarea class="comment" rows="4" cols="50" id="commentaire_f" name="commentaire_f" title="Commentaire sur le fournisseur"><?php
                        if (isset($_SESSION['commentaire_f']))
                            echo $_SESSION['commentaire_f']; if (isset($_GET['id'])) {
                            echo $tab_fournisseur["comment_fournisseur"];
                        }
                                ?></textarea>
                </fieldset>
            </div>
...
?>
                    ...
                    <input id="check_physique" type="checkbox" title="Existe t-il un dossier physique?"></input>
