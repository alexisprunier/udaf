...
$('.searchAutocomplete').click(function() {
 ...
 function Processgraph(){
	...
	// Textarea limit
	 $('#Textarealimit').limit('250','.limitchars');
	...
	if(mybrowser.indexOf('Safari')>0){}		
	  ...
	  function fileDialog(callback,type){
		...
		$.confirm({
		'title': 'Alerte de suppression','message': " <strong>Voulez-vous vraiment supprimer </strong><br /><font color=red>' "+ name +" ' </font> ",'buttons': {'Oui': {'class': 'special',
		'action': function(){
					loading('Checking');
									 $('#preloader').html('Suppression...');
