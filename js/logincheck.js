$(document).ready(function () {	  
			...
			$('.logo').show().delay(1200).animate({   opacity: 1,top: '3%' }, 300,function(){
	...
	$('#but_login').click(function(e){				
		  ...
		  {
			  showError("Merci de renseigner le login / mot de passe !");
	...
	$('form:eq(0)').submit(function(e){	
		...
		$.post(window.location.href,$('form:eq(0)').serialize(),function(data){				
			...
			{
						showError("Mauvais Login / Mot de passe ! ");
						return false;
			}
			hideTop();
			Login();
...
function Login(){
	...
	$("#login").animate({   opacity: 1,top: '49%' }, 200,function(){
		 $('.userbox').show().animate({ opacity: 1 }, 500);
			...
			setTimeout( "window.location.href='accueil.php?module=materiel&action=lister_materiel'", 3000 );
}
